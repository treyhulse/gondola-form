...
# Set up the layout with two columns: left for form, right for image (1:1.5 ratio)
...
# Left Column: Form (50% of the screen width)
with col1:
    ...
    side_a_panel = st.radio(
        "", ["P", "M"], index=0, horizontal=True, on_change=update_image
    )

    # Side B Back Panel Type
    st.subheader("Side B Back Panel Type")
    side_b_panel = st.radio(
        "", ["P", "M", "None"], index=0, horizontal=True, on_change=update_image
    )
    ...
    selected_base_depth_a = st.select_slider(
        "", options=base_depth_a_options, value=base_depth_a_options[0], on_change=update_image
    )
    ...
    upper_shelf_qty = st.slider(
        "", min_value=0, max_value=6, step=1, value=0, on_change=update_image
    )

    # Upper Shelf Type
    st.subheader("Upper Shelf Type")
    upper_shelf_type = st.radio(
        "", ["TL", "DL"], index=0, horizontal=True, on_change=update_image
    )
    ...
    selected_upper_shelf_depth = st.select_slider(
        "", options=upper_shelf_depth_options, value=upper_shelf_depth_options[0], on_change=update_image
    )

    # Base Height
    st.subheader("Base Height")
    base_height = st.radio(
        "", ["LB", "06"], index=0, horizontal=True, on_change=update_image
    )
